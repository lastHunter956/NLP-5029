...
_ALL_POS = POSITIVE_WORDS | POSITIVE_EMOJIS
_ALL_NEG = NEGATIVE_WORDS | NEGATIVE_EMOJIS
...
class SentimentFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    original_texts : list or None
        Lista de textos originales (antes de limpieza) para calcular
        all_caps_ratio e is_question con el texto raw. Si None, se
        infiere del texto preprocesado (menos preciso).
    """
    ...
    def transform(
        ...
        for i, text in enumerate(X):
            raw = orig[i] if (orig is not None and i < len(orig)) else None
    ...
    def _extract(self, text: str, raw_text: Optional[str] = None) -> List[float]:
        ...
        # ── 4. negation_count ─────────────────────────────────────
        neg_count = sum(1 for t in tokens if t in NEGATORS) / n
        ...
        # ── 6. intensifier_count ──────────────────────────────────
        intens = sum(1 for t in tokens if t in INTENSIFIERS) / n

        # ── 7. attenuator_count ───────────────────────────────────
        attenu = sum(1 for t in tokens if t in ATTENUATORS) / n

        # ── 8. emoji_pos_ratio ────────────────────────────────────
        emoji_pos = tokens.count('emoji_pos') / n

        # ── 9. emoji_neg_ratio ────────────────────────────────────
        emoji_neg = tokens.count('emoji_neg') / n
        ...
        pos_big = sum(1 for b in bigrams if b in POSITIVE_BIGRAMS) / max(len(bigrams), 1)

        # ── 12. neg_bigram_score ──────────────────────────────────
        neg_big = sum(1 for b in bigrams if b in NEGATIVE_BIGRAMS) / max(len(bigrams), 1)
        ...
        for idx, t in enumerate(tokens):
            if t in NEGATORS or t.startswith('no_'):
